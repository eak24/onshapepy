...
def test_encode_configuration(client):
    s = client.encode_configuration(did, eid, {"Length": "5 meters", "height": "2 meters"})
    assert s == 'Length=5+meters;height=2+meters'

def test_rename_document(client):
    res = client.rename_document(did, "Test")
    assert res.status_code == 200

def test_insert_configured_part(client):
    res = client.create_assembly_instance(assembly, part_in_another_doc, {"h": "3 m"})
    assert res.status_code == 200

def test_get_configuration(client):
    res = client.get_configuration(part)
    assert res.status_code == 200